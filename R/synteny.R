# README
# This script processes GFF and MAF files to combine information and create a GRanges object for further analysis.
# The script performs the following steps:
# 1. Installs and loads the necessary packages.
# 2. Reads the GFF and MAF files.
# 3. Extracts and combines information from the GFF and MAF files.
# 4. Creates a GRanges object from the combined data.
# 5. Prints debugging information to verify the processing steps.

# Author: Bloodmark

# Input details:
# - GFF file: "/home/bloodmark/workarea/sibeliaz_out/blocks_coords.gff"
# - MAF file: "/home/bloodmark/workarea/sibeliaz_out/alignment.maf"

# Output details:
# - Combined information printed for verification.
# - GRanges object created from the combined data.

# Script functioning:
# 1. Install and load the required packages (GenomicRanges and circlize).
# 2. Read the GFF and MAF files.
# 3. Extract information from the MAF file.
# 4. Combine information from the GFF and MAF files.
# 5. Create a GRanges object from the combined data.
# 6. Print debugging information to verify the processing steps.
...
for (i in seq_along(combined_data)) {
  ...
  cat("\n")
  if (i == 5) break  # Print only the first 5 entries 
}
